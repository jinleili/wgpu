...
- 当前分支：`codex/issue-8518-texture-folding`；开始时 HEAD 为 `8c91d7a80a814d8641b2b69862e4ae37a3e11a1b`，工作区干净。该提交仅添加此前正确性报告。
...
& "$r/binaries/a/wgpu-benchmark.exe" --bench --exact 'Texture Init Render: shared attachments' --iters 128 --color never --save-baseline vulkan-aa-g1-s1-p1-a
```
...
每次用唯一 `--save-baseline <run-id>`，随后立即复制该 JSON 到 `raw/<run-id>.json`，stdout、stderr、命令、开始结束时间和退出码分别保存。框架仍更新 `previous.json`，但历史结果不依赖它；终端自动比较百分比不参与分析。检查每进程确实输出一个场景、三项指标、每项 128 次迭代及 10000 draws/1000 groups，退出码 0 且实际 adapter 匹配。
...
```powershell
node "$r/schedule.cjs"
& "$r/run.ps1" -Backend vulkan -Phase preflight
& "$r/run.ps1" -Backend dx12 -Phase preflight
& "$r/health.ps1" -Label before-vulkan-aa
& "$r/run.ps1" -Backend vulkan -Phase aa
node "$r/analyze.cjs"
& "$r/health.ps1" -Label before-vulkan-ab
& "$r/run.ps1" -Backend vulkan -Phase ab
node "$r/analyze.cjs"
& "$r/health.ps1" -Label before-dx12-aa
& "$r/run.ps1" -Backend dx12 -Phase aa
node "$r/analyze.cjs"
& "$r/health.ps1" -Label before-dx12-ab
& "$r/run.ps1" -Backend dx12 -Phase ab
node "$r/analyze.cjs"
node "$r/report.cjs"
```

没有因显著性不足追加样本，也没有移除不利结果。A/A 观测漂移是本次环境下的有限经验参照，不是置信区间或未来噪声上界。
